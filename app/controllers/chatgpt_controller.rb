class ChatgptController < ApplicationController
	...
	def generate
        ...
        parameters: {
            model: "gpt-3.5-turbo",
            messages: [{ role: "user", content: "#{params["search"]} + only html code"}],
        })
    ...
    def suggest
        response = HTTParty.get('http://suggestqueries.google.com/complete/search?output=firefox&hl=en&q=delhi'
      )
